function peopleAnimation({
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      if (whiteCells.has(`${r}-${c}`)) {
        cell.className = `w-full h-full md:w-[4rem] md:h-[3.12rem] lg:w-[3.7rem] lg:h-[2.75rem] ${whiteClass}`;
      ...
      cell.className = `w-full h-full md:w-[4rem] md:h-[3.12rem] lg:w-[3.7rem] lg:h-[2.75rem] ${
